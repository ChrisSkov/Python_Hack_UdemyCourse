...
path_to_log = '/home/kali/Desktop/log.txt'
    ...
    for line in my_file:
        for item in sus[0:len(sus)]:
            if item in line:
                ...
                country = pycountry.countries.get(alpha_2=line_to_print[1].country)
                if country.name not in my_dict:
                    my_dict[country.name] = 1
                else:
                    my_dict[country.name] = my_dict[country.name] + 1
                break
        lines += 1
...
def find_info_from_ip(line):
    ...
    result = (line_to_print, request_info)
    return result
